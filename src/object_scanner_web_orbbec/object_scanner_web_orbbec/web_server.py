...
from object_scanner_web_orbbec.sqlite_recording import validated_session_name
from object_scanner_web_orbbec.transformations import (
...
REGISTERED_DEPTH_TOPIC = "/camera/depth/image_raw"
...
SERVICE_NAMES = {
    ...
    "pause": "/object_scanner_orbbec/pause_recording",
    "resume": "/object_scanner_orbbec/resume_recording",
    "stop": "/object_scanner_orbbec/stop_recording",
}
...
class RosControlBridge(Node):
    """Call the Orbbec scanner services from Flask request threads."""

    def __init__(self, transformation_path: Path | None = None) -> None:
        super().__init__("object_scanner_web_orbbec")
    ...
    def _set_scanner_parameter_unlocked(
        ...
        if not self._parameter_client.wait_for_services(
            ...
            raise TimeoutError(
                "/object_scanner_orbbec parameter services are unavailable"
            )

        future = self._parameter_client.set_parameters([parameter])
        response = self._wait_for_future(
            future,
            "/object_scanner_orbbec/set_parameters",
        )
        if response is None or not response.results:
            raise RuntimeError(
                "/object_scanner_orbbec/set_parameters returned no result"
            )
    ...
    def _on_synchronized_sensor_frame(
        ...
        try:
            ...
            color_from_pointcloud = np.eye(4, dtype=np.float64)
            ...
            transformation = TransformationMatrix(
                ...
                matrix=tuple(
                    ...
                    for row in calibration.camera_to_world
                ),
            )
            transform_message = transformation_to_message(
                transformation,
    ...
    def _calibrate_sensor_frame(
        ...
        if color_image.header.frame_id != frame_id:
            raise CharucoCalibrationError(
                "RGB image frame "
                f"'{color_image.header.frame_id}' does not match point-cloud "
                f"frame '{frame_id}'"
            )
        if camera_info.header.frame_id != frame_id:
            raise CharucoCalibrationError(
                "CameraInfo frame "
                f"'{camera_info.header.frame_id}' does not match point-cloud "
                f"frame '{frame_id}'"
            )
        if (
...
def create_app(
    ...
    if share_directory is None:
        share_directory = Path(
            get_package_share_directory("object_scanner_web_orbbec")
        )
    app = Flask(
        __name__,
...
def main(args=None) -> None:
    ...
    executor_thread = threading.Thread(
        ...
        name="object-scanner-web-orbbec-ros",
        daemon=True,
    )
