...
fuses all committed frames before the service call returns. ChArUco or JSON
camera-to-world matrices provide the initial poses. Registration is
accepted only for overlapping frames when the point-to-plane ICP residual
improves and its correction remains within 10 mm and 2 degrees. Failure is
reported instead of silently displaying an unsafe raw merge.

The hardware-independent algorithm and its Open3D dependency are owned by the
shared `object_scanner_processing` package and are also used by the RealSense
web scanner.
...
and matrix; replay endpoints continue to show those original per-frame
captures.
...
While ChArUco mode is selected, its panel shows the latest RGB frame with
detected marker borders and ChArUco corners drawn over it. The browser refreshes
this diagnostic preview at no more than 5 FPS, and it remains active while the
recorder is stopped, recording, or paused. Preview frames are not saved and are
not reused by Capture.

Start recording, keep the board visible, and click **Capture**. The server uses
the RGB image synchronized to that registered point cloud and the intrinsics
from `/camera/color/camera_info` to calculate a fresh camera-to-world matrix.
The image, CameraInfo, and point cloud must share the same frame ID and image
dimensions. The current launch configuration normally supplies all three in
...
- `GET /api/points` while paused or stopped; returns the cleaned, registered,
  fused preview
- `GET /api/sessions`
